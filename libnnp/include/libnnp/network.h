...
public:
	...
	static constexpr size_t outputCount() {
		return LayerType<layerCount() - 1>::nodeCount();
	}

	static constexpr size_t inputCount() {
		return LayerType<0>::inputCount();
	}

	template <typename Next, typename InputFloat, size_t BATCH_SIZE = RESIZEABLE>
	Tensor<InputFloat, inputCount(), BATCH_SIZE> propagate(Next&& next
		, const Tensor<InputFloat, inputCount(), BATCH_SIZE>& input
		, InputFloat stepSize, InputFloat regularization) {
		return PropagateHelper<0>()(this, next, input, InputFloat{0}, stepSize, regularization);
	}

	template <typename Next, typename InputFloat, size_t BATCH_SIZE = RESIZEABLE>
	void propagate(Next&& next
		, const Tensor<InputFloat, inputCount(), BATCH_SIZE>& input, InputFloat regularization) {
...
private:
	...
	template <size_t LAYER_IDX, typename Dummy = void> // Only partial specializations are allowed in class scope.
	struct PropagateHelper {
		template <typename Next, typename InputFloat, size_t BATCH_SIZE>
		Tensor<InputFloat, LayerType<LAYER_IDX>::inputCount(), BATCH_SIZE>
			operator ()(TupleNetwork* object, Next&& next
				, const Tensor<InputFloat, LayerType<LAYER_IDX>::inputCount(), BATCH_SIZE>& input
				, InputFloat totalL2Norm, InputFloat stepSize, InputFloat regularization) const {
			auto& thisLayer = object->getLayer<LAYER_IDX>();
			auto output = thisLayer.forward(input);
			totalL2Norm += thisLayer.l2Norm();
			auto gradient = PropagateHelper<LAYER_IDX + 1, Dummy>()(object, next, output, totalL2Norm, stepSize, regularization);
		...
		template <typename Next, typename InputFloat, size_t BATCH_SIZE>
		void operator ()(TupleNetwork* object, Next&& next
				, const Tensor<InputFloat, LayerType<LAYER_IDX>::inputCount(), BATCH_SIZE>& input
				, InputFloat totalL2Norm, InputFloat regularization) const {
			auto& thisLayer = object->getLayer<LAYER_IDX>();
			PropagateHelper<LAYER_IDX + 1, Dummy>()(object, next, thisLayer.forward(input), totalL2Norm, regularization);
		}
	};
	...
	struct PropagateHelper<layerCount() - 1, Dummy> {
		template <typename Next, typename InputFloat, size_t BATCH_SIZE>
		Tensor<InputFloat, LayerType<layerCount() - 1>::inputCount(), BATCH_SIZE>
			operator ()(TupleNetwork* object, Next&& next
				, const Tensor<InputFloat, LayerType<layerCount() - 1>::inputCount(), BATCH_SIZE>& input
				, InputFloat totalL2Norm, InputFloat stepSize, InputFloat regularization) const {
	...
	struct ForwardHelper {
		...
		Tensor<InputFloat, LayerType<LAYER_IDX>::nodeCount(), BATCH_SIZE>
			operator ()(TupleNetwork* object, const Tensor<InputFloat, TupleNetwork::inputCount(), BATCH_SIZE>& input) const {
			auto& thisLayer = object->getLayer<LAYER_IDX>();
	...
	struct ForwardHelper<0, Dummy> {
		template <typename InputFloat, size_t BATCH_SIZE>
		Tensor<InputFloat, LayerType<0>::nodeCount(), BATCH_SIZE>
			operator ()(TupleNetwork* object, const Tensor<InputFloat, TupleNetwork::inputCount(), BATCH_SIZE>& input) const {
	...
	template <size_t IDX>
	constexpr auto& getLayer() { return std::get<IDX>(m_layers); }

	template <size_t IDX>
	constexpr const auto& getLayer() const { return std::get<IDX>(m_layers); }
};
...
public:
	...
	auto propagate(const Tensor<InputFloat, inputCount(), BATCH_SIZE>& input
...
private:
	...
	public:
		...
		auto propagate(const Tensor<InputFloat, HLayers::outputCount(), BATCH_SIZE>& input
			, InputFloat totalL2Norm, InputFloat, InputFloat regularization) {
		...
		template <typename InputFloat>
		void propagate(const Tensor<InputFloat, HLayers::outputCount(), BATCH_SIZE>& input
			, InputFloat totalL2Norm, InputFloat regularization) {
			auto probs = m_lossLayer->probs(input);
			m_loss = m_lossLayer->loss(probs, *m_groundTruth, totalL2Norm, regularization);
		}
