...
public:
	...
	template <
		size_t SIZE_D = SIZE,
		size_t BATCH_SIZE_D = BATCH_SIZE,
	...
	decltype(auto) operator()(size_t row, size_t column) { return m_data(row, column); }

	decltype(auto) operator()(size_t row, size_t column) const { return m_data(row, column); }

	size_t size() const { return m_data.nr(); }

	size_t batchSize() const { return m_data.nc(); }

	template <size_t SIZE_D = SIZE, typename = std::enable_if_t<SIZE_D == RESIZEABLE>>
