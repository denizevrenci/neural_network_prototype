...
template <typename Float, size_t SIZE_E, size_t BATCH_SIZE_E
	, size_t SIZE_G, size_t BATCH_SIZE_G>
Float crossEntropy(const Tensor<Float, SIZE_E, BATCH_SIZE_E>& estimation
	, const Tensor<Float, SIZE_G, BATCH_SIZE_G>& groundTruth) {
	assert(estimation.size() == groundTruth.size()
...
public:
	...
	static Tensor<Float, SIZE, BATCH_SIZE> probs(const Tensor<InputFloat, SIZE, BATCH_SIZE>& input) {
		return softmax(input);
	}

	template <typename PFloat, typename GFloat, size_t SIZE, size_t BATCH_SIZE>
	static Float loss(const Tensor<PFloat, SIZE, BATCH_SIZE>& probs
		, const Tensor<GFloat, SIZE, BATCH_SIZE>& groundTruth
		, Float totalL2Norm, Float regularization) {
